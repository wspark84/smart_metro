...
export function buildDepartureReminder(plan, now, routeNumber = '') {
  ...
  const basis = estimated ? '배차간격 예상' : '실시간 도착정보 기준';
...
export function buildBoardingPlan({ now, requiredArrivalTime, route, arrivalsMin = [], snapshot,
  ...
  const recent = Number.isFinite(age) && age >= 0 && age <= 30;
  ...
  // Bound extrapolation to today's target and six hours; never roll into tomorrow.
  if (duration && interval && anchor !== null && latestMin >= 0 && latestMin <= 360) {
    let next = anchor + interval;
    if (next < 0) next += Math.ceil(-next / interval) * interval;
    for (let count=0; next <= latestMin + interval && count<182; count++, next+=interval) {
